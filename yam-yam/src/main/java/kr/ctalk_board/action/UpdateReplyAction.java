...
public class UpdateReplyAction implements Action{
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		long ctalk_re_num = Long.parseLong(request.getParameter("ctalk_re_num"));
		
		CTALK_BoardDAO dao = CTALK_BoardDAO.getInstance();
		Ctalk_Re_BoardVO db_reply = dao.getReplyBoard(ctalk_re_num);
		...
		if (user_num == null) { // 로그인이 되지 않은 경우
			...
			reply.setCtalk_re_num(ctalk_re_num);
