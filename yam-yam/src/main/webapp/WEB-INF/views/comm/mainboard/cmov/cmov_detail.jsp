...
<head>
<meta charset="UTF-8">
<title>오늘 '뭐' 보지?글 상세</title>
<link rel="stylesheet" href="${pageContext.request.contextPath}/css/style.css" type="text/css">
<link rel="stylesheet" href="${pageContext.request.contextPath}/css/common.css" type="text/css">
<script type="text/javascript" src="${pageContext.request.contextPath}/js/jquery-3.7.1.min.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/js/cmov.reply.js"></script>
</head>
<body>
	<div class="page-main">
		...
		<div class="content-main">
			...
			<h2 class="fw-700">${cmov.cmov_title}</h2>
			<ul class="">
				<li class="fw-700 fs-12">
					...
					<c:if test="${empty board.photo}">
						...
						<span class="va-super">${cmov.mem_nickname}</span>
					</a>
					</li>
					<li class="fs-08 text-r">
						<span>조회 : ${cmov.cmov_hit}</span>
						<span class="ml-2"><c:if test="${!empty cmov.cmov_date}"></c:if> 작성일 : ${cmov.cmov_date}</span>
					</li>
				</ul>
			...
			<article class="p-2">
				...
				링크 : ${cmov.cmov_link}
				<br>
				내용 : ${cmov.cmov_article}
				</article>
				
			<div class="flex-box f-center p-1">
					<%-- 로그인한 회원번호와 작성자 회원번호가 일치해야 수정, 삭제 가능 --%> 
					<c:if test="${user_num == cmov.mem_num}">
						<input type="button" value="수정"onclick="location.href='updateForm.do?cmov_num=${cmov.cmov_num}'" class="btn btn-line-primary mr-1">
						...
						<script type="text/javascript">
							...
							delete_btn.onclick = function() {
								...
								if (choice) {
									location
											.replace('delete.do?cmov_num=${cmov.cmov_num}');
								}
							};
						</script>
					</c:if>
