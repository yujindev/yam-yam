		...
		<div class="content-main">
			...
			<ul class="">
				...
				<li class="fs-08 text-r">
					<span>조회 : ${ctalk.ctalk_hit}</span>
					<span class="ml-2"><c:if test="${!empty ctalk.ctalk_date}"></c:if> 작성일 : ${ctalk.ctalk_date}</span>
				</li>
			</ul>
					...
					<c:if test="${user_num == ctalk.mem_num}">
						...
						<script type="text/javascript">
							...
							delete_btn.onclick = function() {
								...
								if (choice) {
									location
											.replace('delete.do?ctalk_num=${ctalk.ctalk_num}');
								}
							};
						</script>
					</c:if>
			...
			<div id="reply_div">
				...
				<form id="re_form" class="w-90 flex-box f-between position-r m-0auto">
					<input type="hidden" name="ctalk_num" value="${ctalk.ctalk_num}"
						id="ctalk_num">
					<textarea rows="3" cols="50" name="ctalk_re_content" id="ctalk_re_content" class="rep-content w-80" <c:if test="${empty user_num}"> disabled = "disabled" placeholder="로그인 해야 작성할 수 있습니다."</c:if>></textarea>
