...
<body>
	<div class="page-main">
    	...
    	<h2 class="fw-700 fs-16 m-1">맛집추천 게시판</h2>
    	...
    	<div class="list-space flex-box f-end m-1">
    		...
    		<input type="button" value="글쓰기" onclick="location.href='cmenu_writeForm.do'" class="btn-re btn-line-primary ">
    		</c:if>
    		...
    		<c:if test="${count > 0}">
    			<ul class="w-95 m-0auto">
		             <c:forEach var="cmenu" items="${list}">
                		<li class="mt-2">
            		    	<a href="cmenu_detail.do?cmenu_num=${cmenu.cmenu_num}">
		                     <p class="fs-10 text-main">${cmenu.cmenu_num}</p>
		                     <p class="fs-12 fw-700 mt-1">${cmenu.cmenu_title}</p>
		                     <p class="fs-08 mt-1">${cmenu.cmenu_date}</p>
		                     <p class="fs-08 text-r">조회수 | ${cmenu.cmenu_hit}</p>
