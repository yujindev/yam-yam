$(function(){
	...
	*	댓글 목록
	...
	$.ajax({
		url:'listReply.do',
		type :'post',
		data:{pageNum:pageNum, rowCount:rowCount, czone_num:$('#czone_num').val()},
		dataType :'json',
...
$('.paging-button input').click(function(){
	...
	*  =========================*/
	...
	$('#re_form').submit(function(event){
		if($('#czone_re_content').val().trim()==''){
			alert('내용을 입력해라');
			$('#czone_re_content').val('').focus();
			return false;
		}
	...
	function initForm(){
			$('textarea').val('');
			$('#czone_re_first.letter-count').text('300/300');
	}
	/* =========================
	*	댓글 수정
	*  =========================*/
	...
	$(document).on('click','.modify-btn',function(){
		...
		// 입력한 글자수 셋팅
		let inputLength = $('#czone_mre_content').val().length;
	...
	$(document).on('submit', '#mre_form', function(event){
	            if($('#czone_mre_content').val().trim()==''){
	                alert('내용을 입력하세요!');
	                $('#czone_mre_content').val('').focus();
	                return false;
	            }
	            //폼에 입력한 데이터 반환
	            let form_data = $(this).serialize();
	                ...
	                success:function(param){
	                    if(param.result =='logout'){
	                        ...
	                        $('#mre_form').parent().find('p').html(
	                            $('#czone_mre_content').val().replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g, '<br>'));
	...
	*  =========================*/
	$(document).on('keyup', 'textarea', function(){
	       ...
	       if(inputLength>300){//300자가 넘어선 경우
	           ...
	           let remain = 300 - inputLength;
	           remain +='/300';
	           if($(this).attr('id') == 'czone_re_content'){
	               //등록폼 글자수
