	...
	$.ajax({
		...
		success:function(param){
			...
			$(param.list).each(function(index,item){
				...
				output += '<h4>' + item.cbob_num + '</h4>';
				output += '<div class = "sub-item">';
				output += '<p>' + item.cbob_re_content + '</p>';

				if(item.cbob_re_mdate){
					output += '<span class = "modify-date"> 최근 수정일 :' + item.cbob_re_mdate + ' </span>';
				} else{
					output += '<span class = "modify-date"> 등록일 :' + item.cbob_re_date + ' </span>';
				}
				
				if(param.user_num == item.mem_num){
					output += ' <input type="button" data-renum="'+item.cbob_re_num+'" value="수정" class="modify-btn">'
					output += ' <input type="button" data-renum="'+item.cbob_re_num+'" value="삭제" class="delete-btn">'
				}
...
$('.paging-button input').click(function(){
	...
	*  =========================*/
	...
	$(document).on('click','.modify-btn',function(){
		...
		modifyUI += '<textarea rows="3" cols="50" name="cbob_re_content" id="cbob_mre_content" class="rep-content">'+cbob_re_content+'</textarea>';
